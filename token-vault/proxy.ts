...
import { cloneResponse, createErrorResponse, generateUrls } from "./utils/network.utils";
import { ProxyConfig, ServerTokens } from "./interface";
...
export function proxy(config: ProxyConfig) {
  ...
  window.addEventListener("message", async (event) => {
    ...
    if (requestType === hasTokenEventName) {
      ...
      const hasToken: boolean = Boolean(tokens?.accessToken);
    ...
    if (!tokens) {
      const errorResponse = createErrorResponse("no_tokens", new Error("No OAuth/OIDC tokens found"));
      responseChannel.postMessage(errorResponse);
      return;
    }
